...
class Model(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.backbone = nn.ModuleList([TransformerLayer(config.embedding_dim, config.num_heads, config.intermediate_dim) for _ in range(config.num_backbone_layers)])
        self.decoder = nn.ModuleList([TransformerLayer(1024, config.num_heads, 4096) for _ in range(config.num_decoder_layers)])
...
class CUDAGenerator(Generator):
    ...
    def generate_frame(self, tokens: torch.Tensor, tokens_mask: torch.Tensor, input_pos: torch.Tensor, temperature: float, topk: int):
        ...
        backbone_q_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_q_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_q_proj_weights])
        backbone_k_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_k_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_k_proj_weights])
        backbone_v_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_v_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_v_proj_weights])
        backbone_out_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_out_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_out_proj_weights])
        backbone_q_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_q_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_q_proj_biases])
        backbone_k_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_k_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_k_proj_biases])
        backbone_v_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_v_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_v_proj_biases])
        backbone_out_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_out_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_out_proj_biases])
        backbone_mlp_w1_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_w1_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_mlp_w1_weights])
        backbone_mlp_w2_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_w2_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_mlp_w2_weights])
        backbone_mlp_w1_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_w1_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_mlp_w1_biases])
        backbone_mlp_w2_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_w2_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_mlp_w2_biases])
        backbone_sa_norm_w_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_sa_norm_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_sa_norm_weights])
        backbone_mlp_norm_w_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_norm_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.backbone_mlp_norm_weights])
        backbone_sa_norm_b_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_sa_norm_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_sa_norm_biases])
        backbone_mlp_norm_b_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.backbone_mlp_norm_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.backbone_mlp_norm_biases])
        lib.transformer_decoder_forward_cuda(
            ...
            backbone_q_proj_ptrs,
            backbone_k_proj_ptrs,
            backbone_v_proj_ptrs,
            backbone_out_proj_ptrs,
            backbone_q_bias_ptrs,
            backbone_k_bias_ptrs,
            backbone_v_bias_ptrs,
            backbone_out_bias_ptrs,
            backbone_mlp_w1_ptrs,
            backbone_mlp_w2_ptrs,
            backbone_mlp_w1_bias_ptrs,
            backbone_mlp_w2_bias_ptrs,
            backbone_sa_norm_w_ptrs,
            backbone_mlp_norm_w_ptrs,
            backbone_sa_norm_b_ptrs,
            backbone_mlp_norm_b_ptrs,
        ...
        for i in range(1, self.model.config.audio_num_codebooks):
            ...
            decoder_q_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_q_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_q_proj_weights])
            decoder_k_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_k_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_k_proj_weights])
            decoder_v_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_v_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_v_proj_weights])
            decoder_out_proj_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_out_proj_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_out_proj_weights])
            decoder_q_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_q_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_q_proj_biases])
            decoder_k_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_k_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_k_proj_biases])
            decoder_v_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_v_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_v_proj_biases])
            decoder_out_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_out_proj_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_out_proj_biases])
            decoder_mlp_w1_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_w1_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_mlp_w1_weights])
            decoder_mlp_w2_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_w2_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_mlp_w2_weights])
            decoder_mlp_w1_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_w1_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_mlp_w1_biases])
            decoder_mlp_w2_bias_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_w2_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_mlp_w2_biases])
            decoder_sa_norm_w_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_sa_norm_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_sa_norm_weights])
            decoder_mlp_norm_w_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_norm_weights))(*[ctypes.cast(w.data_ptr(), ctypes.POINTER(ctypes.c_float)) for w in self.decoder_mlp_norm_weights])
            decoder_sa_norm_b_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_sa_norm_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_sa_norm_biases])
            decoder_mlp_norm_b_ptrs = (ctypes.POINTER(ctypes.c_float) * len(self.decoder_mlp_norm_biases))(*[ctypes.cast(b.data_ptr(), ctypes.POINTER(ctypes.c_float)) for b in self.decoder_mlp_norm_biases])
            lib.transformer_decoder_forward_cuda(
                ctypes.cast(curr_h_proj.contiguous().data_ptr(), ctypes.POINTER(ctypes.c_float)),
                ...
                decoder_k_proj_ptrs,
                decoder_v_proj_ptrs,
                decoder_out_proj_ptrs,
                decoder_q_bias_ptrs,
                decoder_k_bias_ptrs,
                decoder_v_bias_ptrs,
                decoder_out_bias_ptrs,
                decoder_mlp_w1_ptrs,
                decoder_mlp_w2_ptrs,
                decoder_mlp_w1_bias_ptrs,
                decoder_mlp_w2_bias_ptrs,
                decoder_sa_norm_w_ptrs,
                decoder_mlp_norm_w_ptrs,
                decoder_sa_norm_b_ptrs,
                decoder_mlp_norm_b_ptrs,
    ...
    def generate(self, text, speaker, context, max_audio_length_ms):
        ...
        if not text_tokens:
            return torch.randn(int(self.sample_rate * max_audio_length_ms / 1000), device=self.device)
        batch_size = 1
